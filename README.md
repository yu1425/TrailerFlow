...
| 値 | 動作 | 推奨用途 |
| --- | --- | --- |
| `manual` | `contents` テーブルの approved データのみ | **公開 β（本番必須）** |
| `tmdb` | 従来の `movies` / `trailers` テーブルのみ | 個人ローカル検証のみ |
| `mixed` | approved contents を優先し、不足分を TMDb で補填 | 開発中のローカル比較用 |

> **本番（Vercel Production）では `DATA_MODE=manual` を絶対に変更しないでください。** TMDb personal API key は個人利用向けであり、公開サービスのフィード供給に使用すると利用規約違反になる可能性があります。`tmdb` / `mixed` はローカル環境での比較・候補収集にのみ使用してください。
...
- 公開 β では全コンテンツが日本語キュレーション済みのため、言語トグルは非表示です。英語コンテンツが増えた段階で復活予定。
